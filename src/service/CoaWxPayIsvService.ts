...
export class CoaWxPayIsvService {
  ...
  getPaymentParams(data: { appWxaId: string; prepayId: string }) {
    const prepayId =
      data.prepayId ||
      CoaError.message('CoaWxPayIsv.MissingField', '缺少prepayId')
    const param: any = {
      appId:
        data.appWxaId ||
        CoaError.message('CoaWxPayIsv.MissingField', '缺少appWxaId'),
  ...
  async unifiedOrder(data: {
    ...
    const param = {
      ...
      sub_mch_id:
        data.subMchId ||
        CoaError.message('CoaWxPayIsv.MissingField', '缺少subMchId'),
      sub_openid:
        data.openId ||
        CoaError.message('CoaWxPayIsv.MissingField', '缺少openId'),
      body: data.body || '订单' + data.orderId,
      out_trade_no:
        data.orderId ||
        CoaError.message('CoaWxPayIsv.MissingField', '缺少orderId'),
      total_fee:
        data.price ||
  ...
  async payRefund(data: {
    ...
    const orderId =
      data.orderId || CoaError.message('CoaWxPayIsv.MissingField', '缺少订单ID')
    const totalPrice =
      data.totalPrice ||
      CoaError.message('CoaWxPayIsv.MissingField', '缺少总价格')
    const refundPrice =
      data.refundPrice ||
    ...
    const param = {
      ...
      sub_appid:
        ...
        rawData.sub_appid ||
        CoaError.message(
          'CoaWxPayIsv.MissingField',
          '缺少rawData.subAppid，暂时无法退款'
        ),
      sub_mch_id:
        rawData.subMchId ||
        rawData.sub_mch_id ||
  ...
  async queryRefund(data: {
    ...
    const param = {
      ...
      out_refund_no:
        data.refundId ||
        CoaError.message('CoaWxPayIsv.MissingField', '缺少refundId'),
    }
    const body = await this.bin.toSignedXmlParams(param)
  ...
  async downloadBill(data: { date: string }) {
    const param = {
      ...
      nonce_str: this.bin.generateNonceString(),
      bill_date:
        data.date || CoaError.message('CoaWxPayIsv.MissingField', '缺少date'),
      bill_type: 'ALL',
    }
    const body = await this.bin.toSignedXmlParams(param)
